  ...
  return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative">
      <div className="max-w-6xl w-full flex flex-col items-center px-6 py-10 overflow-y-auto h-screen">
        ...
        {/* Success */}
        {data && data.success && data.approval && !isLoading && (
          <div className="mt-10 w-full max-w-4xl">
            ...
            <div className="bg-white border-x border-gray-200 p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                ...
                <DetailField
                  label="Loan ID"
                  value={data.approval.loanId}
                  highlight
                />
                <DetailField
                  label="Approval Status"
                  value={data.approval.isApproved ? "Approved" : "Not Approved"}
                />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <DetailField
                  label="Asking Price"
                  value={`${data.approval.askingPrice} ETH`}
                  highlight
                />
                <DetailField
                  label="Interest Rate"
                  value={`${data.approval.modifiedInterestRate}%`}
                />
                <DetailField
                  label="Minted"
                  value={data.approval.isMinted ? "Yes" : "No"}
                />
                <DetailField
                  label="Cancelled"
                  value={data.approval.isCancelled ? "Yes" : "No"}
                />
                <DetailField
                  label="Lender Address"
                  value={data.approval.lenderAddress}
                  mono
